...
@main struct LayoutTest {
    static func main() {
        ...
        precondition(other.items.count == 3 && other.verticalItem?.field == "radar")
        precondition(other.rows.count == 2 && other.rows[0][0].field == "speed" && other.rows[1][0].field == "power")
        let duplicates = DashLayout(text: "radar medium height=50\nradar medium height=75")
        precondition(duplicates.items.count == 1 && duplicates.verticalItem?.heightPercent == 50)
        ...
        precondition(normalized.items.map(\.heightPercent) == [100, 100, 100])
